...
switch ($method) {
	...
	default:
		...
		if($conditions) {
			...
			$where = "WHERE FROM_UNIXTIME(`addtime`) >= DATE_ADD(CURDATE(), INTERVAL -{$days} DAY) AND `type`='inquiry' AND `xp_status`=''";
		}
		...
		if($where){
			...
			$count_sql = "SELECT COUNT(`postid`) as post_count, `inforate`,`landingurl`, `belong` FROM `#@_post` WHERE  FROM_UNIXTIME(`addtime`) >= DATE_ADD(CURDATE(), INTERVAL -{$days} DAY) AND `type`='inquiry' AND `xp_status`='' GROUP BY `belong`,`inforate` order by post_count DESC, `inforate` DESC";
		}
